...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.RD Lists-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
       try:
           ...
           if hasattr(e, 'code'):
               ...
               xbmc.executebuiltin("XBMC.Notification(RD,We failed with error code - "+str(e.code)+",10000,"+icon+")")
           elif hasattr(e, 'reason'):
               addon_log('We failed to reach a server.')
               addon_log('Reason: %s' %e.reason)
               xbmc.executebuiltin("XBMC.Notification(RD,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def Search_loop():
    ...
    if Search_name == '':
        ...
        HTML = Open_Url('http://xray840.startdedicated.net/qsa/RD/albumsearch.txt')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(RD,New source added.,5000,"+icon+")")
        if not url is None:
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(RD,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        xbmc.executebuiltin("XBMC.Notification(RD,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(RD ,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(RD,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
